...
namespace Blind
{
	...
	void SceneHierarchyPanel::OnImGuiDraw()
	{
		...
		m_Context->m_Registry.each([&](auto entityID)
		{
			...
			bool opened = ImGui::TreeNodeEx((void*)(uint64_t)(uint32_t)entity, flags, tag_component.Tag.c_str());
	...
	void SceneHierarchyPanel::DrawComponents(Entity entity)
	{
		...
		if (entity.HasComponent<TransformComponent>())
		{
			if (ImGui::TreeNodeEx((void*)typeid(TransformComponent).hash_code(), ImGuiTreeNodeFlags_DefaultOpen, "Transform"))
			{
				...
				DrawVec3Conrol("Translation", tc.Translation);
				glm::vec3 rotation = glm::degrees(tc.Rotation);
				DrawVec3Conrol("Rotation", rotation);
				tc.Rotation = glm::radians(rotation);
				DrawVec3Conrol("Scale", tc.Scale, 1.0f);
				ImGui::TreePop();
			}
		}
