...
namespace Blind
{
	...
	namespace Utils
	{
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum glFormat, GLenum format, uint32_t width, uint32_t height, int index)
		{
		...
		static bool IsDepthFormat(FramebufferTextureFormat format)
		{
			switch (format)
			{
			case Blind::FramebufferTextureFormat::DEPTH24STENCIL8: return true;
			}
			return false;
		}
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		{
			BLIND_ENGINE_ASSERT(m_ColorAttachments.size() <= 4, "Blind only supports up to 4 color attachments in Framebuffers.");
		...
		BLIND_ENGINE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is not complete!");
	...
	int OpenGLFrameBuffer::ReadPixel(uint32_t attachmentI, int x, int y)
	{
		BLIND_ENGINE_ASSERT(attachmentI < m_ColorAttachments.size(), "Attachment Index out of bounds!");
		glReadBuffer(GL_COLOR_ATTACHMENT0 + attachmentI);
		int pixeldata;
